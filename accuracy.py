...
def run_accuracy_test():
    ...
    # Identify categories with only one member
    single_sample_classes = {cls for cls, count in class_counts.items() if count < 2}
    
    X_filtered, y_filtered = [], []
    if single_sample_classes:
        logging.warning(f"The following categories have less than 2 samples and will be excluded from the test: {', '.join(single_sample_classes)}")
    ...
    # Pipeline with the latest parameters from train_model.py
    ...
    pipeline = Pipeline([
        ('tfidf', TfidfVectorizer(
            ...
            min_df=2, # Caps the number of features
            max_df=0.9, # Now up to 3 words (e.g. "Deutsche Rentenversicherung Bund")
            # sublinear_tf=True, # Stricter: Words that appear in >70% of docs are removed
            max_features=50000, # Dampens frequency (10x "car" is not 10x as important as 1x)
