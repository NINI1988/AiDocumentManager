...
class SubfolderHandler(BaseHandler):
    ...
    def setup(self) -> None:
        ...
        if not self.model:
            raise RuntimeError("Training model not available. Run 'python train_model.py' first.")
    ...
    def handle(self, context: ProcessingContext) -> None:
        ...
        probs = self.model.predict_proba([norm_text])[0]
        
        # Log top 3 categories
        top_indices = probs.argsort()[-3:][::-1]
        for idx in top_indices:
            logging.info(f"  Possible Category: {self.model.classes_[idx]} ({probs[idx]:.2f})")

        best_idx = probs.argmax()
        context.confidence = probs[best_idx]
        context.subfolder = self.model.classes_[best_idx]
